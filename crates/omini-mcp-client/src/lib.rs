pub use rmcp::model::{GetPromptResult, Prompt, ReadResourceResult, Resource, ResourceTemplate};

use async_trait::async_trait;
use rmcp::model::{
    CallToolRequestParams, CallToolResult, ClientInfo, GetPromptRequestParams,
    ReadResourceRequestParams, ServerCapabilities, Tool as RmcpTool,
};
...
pub struct McpCatalog {
    pub tools: Vec<McpServerToolSpec>,
...
impl McpClientSet {
    pub fn new<I>(cwd: PathBuf, servers: I) -> Self
    where
        I: IntoIterator<Item = (String, McpServerConfig)>,
    {
        let services = servers
            .into_iter()
            .map(|(name, config)| (name.clone(), McpService::new(name, config)))
            .collect();
    ...
    pub fn ready_server_tools(&self) -> Vec<McpServerToolSpec> {
        self.services
            .read()
            .expect("MCP services lock poisoned")
            .values()
    ...
    pub fn snapshots(&self) -> Vec<McpServiceSnapshot> {
        let mut snapshots = self
            .services
            .read()
            .expect("MCP services lock poisoned")
            .values()
            .map(|service| McpServiceSnapshot {
                name: service.name.clone(),
                status: service.status,
                last_error: service.last_error.clone(),
...
async fn connect_stdio(
    ...
    tokio::time::timeout(startup_timeout, serve_client(client_info(), transport))
        ...
        .map_err(|_| {
            format!(
                "stdio MCP command `{command}` timed out during initialize after {startup_timeout:?}"
            )
        })?
...
async fn connect_streamable_http(
    ...
    tokio::time::timeout(startup_timeout, serve_client(client_info(), transport))
        ...
        .map_err(|_| {
            format!(
                "MCP HTTP server `{server_name}` timed out during initialize after {startup_timeout:?}"
            )
        })?
...
mod tests {
    ...
    fn client_set_with_services(services: Vec<McpService>) -> McpClientSet {
        McpClientSet {
            cwd: PathBuf::from("."),
            services: RwLock::new(
                services
