...
impl SkillRegistry {
    ...
    pub(crate) fn get(&self, name: &str) -> Option<&SkillSpec> {
...
pub(crate) fn load_skill_registry(cwd: &Path) -> SkillRegistry {
    ...
    skill_dirs.push((SkillSourceKind::Project, cwd.join(".omini").join("skills")));
...
pub(crate) fn render_skill_slash_command_invocation(
    spec: &SkillSpec,
    prompt: Option<&str>,
) -> String {
    render_skill_invocation_inner(spec, prompt, Some("slash_command"))
}
...
fn load_skill_registry_from_dirs(
    skill_dirs: impl IntoIterator<Item = (SkillSourceKind, PathBuf)>,
) -> SkillRegistry {
...
fn parse_skill_directory(
    ...
    let source = match source_kind {
        SkillSourceKind::Project => SkillSource::Project(directory.clone()),
        SkillSourceKind::User => SkillSource::User(directory.clone()),
    };
...
mod tests {
    ...
    fn load_project_skill_registry(root: &Path) -> SkillRegistry {
        load_skill_registry_from_dirs([(SkillSourceKind::Project, root.to_path_buf())])
    }
    ...
    #[test]
    fn slash_command_invocation_marks_source() {
        let registry = load_skill_registry_from_dirs(std::iter::empty());
        let spec = registry.get("commit-message").unwrap();

        let output = render_skill_slash_command_invocation(spec, Some("Draft commits."));

        assert!(output.contains("<skill_name>commit-message</skill_name>"));
        assert!(output.contains("<skill_invocation>"));
        assert!(output.contains("<source>slash_command</source>"));
        assert!(output.contains("<user_prompt>\nDraft commits.\n</user_prompt>"));
    }
...
"#,
        ...
        let spec = parse_skill_directory(&dir, SkillSourceKind::Project).unwrap();
